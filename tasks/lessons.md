...
### L49 — El QA que encuentra bugs es adversarial, con datos reales y con doble lectura (ERR-051)
**Problema:** la suite unitaria (188 verdes) y los flujos guiados de validate_flows no encontraron ninguno de los 7 hallazgos del QA adversarial contra la BD real: precio inventado en una orden, un perfil de $130.000 que nadie pidió, payloads con precio 0. Todos aparecieron recién cuando una IA-cliente conversó libre contra el agente real con clientes/catálogo reales.
**Regla:**
- Antes de mostrar el agente al cliente, correr la batería adversarial contra la BD REAL (no mocks): personas-IA con objetivos realistas, `ALEGRA_ENABLED=false`, spies para capturar lo creado y limpieza selectiva al final. `create_pending_client_review` SIEMPRE bloqueada en QA (inserta en `clients` real).
- El juez-IA solo, no alcanza: marcó BIEN una orden con precio inventado y MAL un flujo correcto (contraentrega "debía escalar"). Toda transcripción se lee también con ojo humano/propio, y todo hallazgo de dinero se verifica contra el catálogo real antes de reportarlo.
- Patrón de diseño confirmado dos veces (ERR-050, ERR-051): el modelo puede ESCRIBIR lo que quiera, pero nada que afecte dinero puede FLUIR como texto libre — todo pasa por catálogo/estructura o se descarta. Los atajos deterministas que interceptan un turno deben verificar que el mensaje no traiga MÁS que lo que el atajo maneja (confirmación + datos en bloque + pago en un solo mensaje es normal en clientes reales).

### L50 — Resolver la LÓGICA del fallo, no la palabra que lo disparó (corrección directa del usuario, 2026-07-11)
**Problema:** los fixes venían atacando el fraseo puntual ("antes de cerrar quiero agregar" → detector de tokens) cuando el usuario señaló que ningún cliente repite las palabras exactas. Los dos patrones LÓGICOS reales detrás de sus tests que rompieron al agente:
1. **Retroceso de paso:** el flujo solo sabe avanzar. Cuando el cliente quiere volver a un paso anterior (agregar un análisis cuando ya se le pregunta el pago, corregir un dato en plena confirmación), el "empuje" del paso actual pisa la intención. La solución no es detectar la frase: es que el empuje de CUALQUIER paso ceda ante la señal semántica de cambio (`user_intent_signal=correction`) del modelo.
2. **Corrección puntual ≠ reinicio:** "quiero cambiar el cliente" (o cualquier dato puntual) NO significa "empezar de cero". La orden en curso se conserva completa; se re-verifica solo lo que el cambio afecta (identidad + dirección) y se re-confirma el resto — nunca re-preguntar dato por dato lo que el cliente ya dio.
**Regla:**
